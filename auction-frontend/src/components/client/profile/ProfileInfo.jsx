...
const ProfileInfo = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (phoneNumber) {
          ...
          const matchedCode = COUNTRY_CODES.find((item) =>
  ...
  const parseAddress = async (address) => {
    ...
    const selectedDistrict = districtRes.data.districts.find(
    ...
    if (form.countryCode === "+84") {
      ...
      if (!phoneRegex.test(phoneNum)) {
        return {
          valid: false,
          message:
            "Please enter a valid Vietnamese phone number (without leading 0).",
        };
      }
    } else {
  ...
  const handleSave = async () => {
    ...
    const selectedProvince = provinces.find(
      (p) => p.code.toString() === location.province
    );
    const selectedDistrict = districts.find(
      (d) => d.code.toString() === location.district
    );
          ...
          {editMode ? (
            <div className="d-flex gap-2 align-items-center">
              ...
              <input
                ...
                onChange={handleChange}
                placeholder={
                  form.countryCode === "+84" ? "987654321" : "Phone number"
                }
              />

              {!user.phoneVerified && !phoneVerification.isVerifying && (
                <button
                  className="btn btn-outline-primary btn-verify-fix"
                  type="button"
                  onClick={handleSendPhoneOtp}
                  disabled={
                    phoneVerification.isLoading || !form.phoneNumber.trim()
                  }
                >
            ...
            <div className="card border-primary shadow-sm">
              <div className="card-body p-4">
                ...
                <p className="card-text text-muted mb-4 fs-6">
                  ...
                  <strong className="text-dark">{getFullPhoneNumber()}</strong>
                </p>
        ...
        {editMode ? (
          <>
            ...
            <div className="col-lg-4">
              ...
              {errors.ward && (
                <div className="invalid-feedback">Ward is required.</div>
              )}
            </div>
